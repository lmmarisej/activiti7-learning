...
const tools = {
  ...
  getUrlParam: function (url) {   // 获取地址栏参数
    var object = {};
    if (url.indexOf("?") != -1) {
      var str = url.split("?")[1];
      var strs = str.split("&");
  ...
  /**
   * 保存bpmn对象
   * @param {object} bpmnModeler bpmn 对象
   */
  saveBpmn(bpmnModeler, deploymentName) {
    bpmnModeler.saveXML({format: true}, function (err, xml) {
      ...
      $.ajax({
        url: publicurl + 'process/definitions/add-by-string',
        type: 'POST',
        dataType: "json",
        data: param,
  ...
  upload(bpmnModeler, container) {
    var FileUpload = document.uploadFileForm.uploadFile.files[0]
    var fm = new FormData()
    fm.append('processFile', FileUpload)
    fm.append('deploymentName', 'deploymentName')
    $.ajax({
      url: publicurl + 'process/definitions/add-by-file',
      type: 'POST',
      data: fm,
      async: false,
      contentType: false, // 禁止设置请求类型
      processData: false, // 禁止 jquery 对 data 数据的处理，默认会处理
      success: function (result) {
        if (result.success) {
